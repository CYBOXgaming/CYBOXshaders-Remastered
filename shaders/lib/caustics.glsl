	...
	#ifdef PROJECTED_CAUSTICS //Makes VL show underwater and makes more accurate caustics
	...
	vec3 waterCaustics(vec3 color, vec4 fpos){
		...
		return color + color * caustics * 0.25 * CAUSTIC_MULT * mix(iswater * land2, 1.0 - (iswater + istransparent), isEyeInWater) * (1.0 - time[1].y * 0.5);
	}
	#endif
#endif