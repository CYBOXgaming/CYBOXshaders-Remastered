...
void main(){
	...
	vec3 bump = (texture2D(normals, adjustedTexCoord.st).rgb * 2.0 - 1.0) + getTerrainHeight(posxz) * 0.1 * (getRainPuddles(wpos)) * smoothstep(0.8,0.9,lightmaps.y) * wetness * clamp(dot(normalize(upPosition), normal),0.0,1.0);
